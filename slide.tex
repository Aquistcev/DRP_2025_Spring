...
\begin{frame}{Root of Unity}
    A complex number $z$ is an $N$th root of unity if $z^N = 1$. The set of $N$th roots of unity is
    $$
...
\begin{frame}{Fast Fourier Transform}
    ...
    Let $\#(M)$ denote the minimum number of operations needed to calculate all the Fourier coefficients, then it takes $\#(M)$ operations to find $a_0, \cdots, a_{M - 1}$. The key is contained in the following recursion step.
\end{frame}
...
\begin{frame}{Polynomial Multiplication}
    ...
    Given polynomials $F$ and $G$, compute their product $F \cdot G$. Assume the degree of $F$ and $G$ are $n$ and $m$.

    \vspace{1em}

    \pause

    If we directly compute every term in $F \cdot G$, the time complexity would be $O(nm)$.
\end{frame}

\begin{frame}{Process}
    First let us extend $F$ and $G$ to a polynomial of degree of $N$ where $N = 2^k$ and $N \ge n + m - 1$ for convenience.
...
\begin{frame}[fragile]{Python Code Implementation}
...
def poly_mul(a, b):
    ...
    C = A * B
    c = np.fft.ifft(C)
    c = np.rint(np.real(c)).astype(int)
    return c[:n+m-1]
